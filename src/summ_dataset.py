# This file is copied from "Clinical Text Summarization by Adapting LLMs | Nature Medicine" at https://github.com/StanfordMIMI/clin-summ.
...
class SummDataset():
    ...
    def load_data(self, args, purpose):
        ...
        if purpose in ['default', 'load_inputs', 'calc_max_new_toks']:
            ...
            if task not in ['trn', 'val', 'test']:
                raise ValueError(
                    'must specify task to be either [trn, val, test]')
        ...
        # get path to results
            ...
            if not os.path.exists(path_data):
                raise FileNotFoundError(
                    f'results not generated for {args.dir_out}')
    ...
    def set_max_new_toks(self, args):
        ...
        if args.thresh_out_toks == 0:  # no thresholding
            ...
            self.max_new_toks = n_toks[int(
                (1 - args.thresh_out_toks) * n_seqs)]
